...
Anschließend wird ein kurzes $\overline{\texttt{STB}}$-Signal erzeugt, dessen negative Flanke ein Flipflop setzen, dessen Ausgangssignal \verb|IBF| anzeigt, dass das Eingaberegister \verb|IB| noch gefüllt ist. Die folgende positive Flanke von $\overline{\texttt{STB}}$ verursacht, die tats"achliche "Ubernahme der Daten ins Eingaberegister \verb|IB|.
...
Die negative Flanke des Lesesignals $\overline{\texttt{RD}}$ f"uhrt zu einer R"ucknahme des Interrupts. Die darauf folgende positive Flanke des Lesesignals f"uhrt anschließend zu einer R"ucknahme des \verb|IBF|-Signals.

\paragraph{Ausgabe}

Das auszugebende Datum wird zuerst auf den Datenbus $\texttt{D}_\texttt{7}, \ldots, \texttt{D}_\texttt{0}$ gelegt. Anschließend wird ein negativer Schreibimpuls $\overline{\texttt{WR}}$ erzeugt. Die folgende positive Flanke f"uhrt dazu, dass das Datum in das Ausgaberegister \verb|DR| "ubertragen wird. Kurze Zeit sp"ater erscheint es auf den Portleitungen $\texttt{PA}_\texttt{7}, \ldots, \texttt{PA}_\texttt{0}$ bzw. $\texttt{PB}_\texttt{7}, \ldots, \texttt{PB}_\texttt{0}$. Außerdem wird ein eventuelles \verb|INT|-Signal gel"oscht und ein Flipflop zur"uckgesetzt, dessen Ausgangssignal (ein nun negatives $\overline{\texttt{OBF}}$) anzeigt, dass ein Datum auf den Portleitungen liegt.
...
Falls das \verb|EI|-Flag der CPU aktiviert ist, beginnt die Interruptvearbeitung: der aktuelle Befehl wird noch fertig ausgef"uhrt. Anschließend wird der mit 4 multiplizierte (da jeder Interrupt-Vektor in diesem Modus aus 4 Bytes besteht) erhaltene Eintrag als Adresse aus der Interruptvektortabelle gelesen und der erhaltene Wert als Interruptvektor (Einsprungadresse in die Interrupt-Service-Routine (ISR)) interpretiert.

Nun wird laut \cite[S.~107]{althaus}  der Wert des Program Counters \verb|PC| und des Flag-Registers auf dem Stack gesichert. Anschließend wird der Interruptvektor in den Program Counter geladen und Interrupts werden deaktiviert (also \verb|EI| ausgeschaltet). Wenn die ISR durch den \verb|IRET|-Befehl verlassen wird, werden Interrupts wieder aktiviert und der Wert des Flasg-Registers und Program Counter wieder hergestellt und somit der regul"are Programmablauf fortgesetzt.
...
Zunächst können Sie Ihre ISR testen, indem Sie mit dem Befehl 
\begin{verbatim}
g=<startadresse> <endadresse>
\end{verbatim}
ausgeführt wird. Da hier iret noch nicht ausgeführt werden darf, muss die
Ausführung davor beendet werden (\verb|endadresse|=Adresse von iret).
...
Das zugeh"orige Oszillogramm sieht folgendermaßen aus. Da eine logische 0 durch ein elektrisches High und eine logische 1 durch ein elektrisches Low codiert wird, haben wir unter dem Vorwissen, dass 36 Bits "ubertragen werden, die Werte in das Oszillogramm eingetragen. Darauf folgt die tabellarische Darstellung der "ubertragenen Bits mit ihrer Bedeutung.
...
Nach jedem "ubertragenen Byte wird ein Parit"atbit bez"uglich der gew"ahlten geraden Parit"at "ubertragenen. Das heißt: ein Bit, welches die Eigenschaft hat, dass die Summe der Datenbits und des Parit"atsbits modulo 2 (bzw. die xor-Verknüpfung dieser) 0 ist.
