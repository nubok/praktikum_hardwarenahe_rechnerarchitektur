...
Da 16 Adressleitungen zur Verf"ugung stehen, ist der Speicherbereich prinzipiell bis zu $2^{16}~\textnormal{Byte}=65536~\textnormal{Byte}=64~\textnormal{KiB}$ groß.
...
Die 31, die in Takt 18 auf dem Datenbus liegt, ist der nach dem Reset vorliegende Wert der Speicherzelle \verb|0000|.
...
9 & M1, MREQ, RD & \verb|0002| & \verb|32|${}^*$ \\
...
Zu der letzten Zeilen eine Bemerkung: es gibt nur 256 Ports, jedoch hat der Adressbus 16 Bits. Daher wird nur das untere Byte des Adressbusses tatsächlich verarbeitet -- im oberen d"urfte theoretisch ein beliebiger Wert stehen, da er ignoriert wird. In der Praxis wird im oberen Byte der Wert des Akkumulators gesendet.
...
Nun legen wir einen \verb|NOP|-Befehl (\verb|00h|), der als Auswirkung nur den Program Counter erhöht, auf  den Datenbus und aktivieren INT.
...
In den ersten acht Schritten setzen wir wie bereits in Abschnitt \ref{sec:nmi_lsg} beschrieben und dort als "uberfl"ussig erkannt, den Interruptmodus auf IM1:
...
Nun ist im Folgenden das Ziel nach dem Betreten der Interrupt Service Routine ein \verb|RETN| abzusetzen, um diese wie in Abschnitt \ref{sec:nmi_lsg} zu verlassen.
