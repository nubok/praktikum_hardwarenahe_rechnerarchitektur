...
{
morekeywords={ORG,CALL,JP,LD,LDE,NOP,RET,RLC,RRC,TITLE,END},
morecomment=[l]{;}
}
...
\begin{figure}
\lstinputlisting[caption={Zugriff auf den RAM},language=z8asm, label=aufgabe31a]{aufgabe31a.asm}
\end{figure}

Listing \ref{aufgabe31a} bildet die Assembler-Lösung der Aufgabe.

In Zeile 5 wird der Port wie in Abschnitt \ref{sec:1a} beschrieben initialisiert. Als nächstes wird in Zeile 6 der Registerpointer (RP) initialisiert. Im weiteren Verlauf sind dann alle verwendeten Register als sogenannte Arbeitsregister deklariert. Durch diese Festlegung ist es möglich, bestimmte Operationen effizienter mit weniger Quelltext auszuführen (Register-Pointer-Befehle).
...
In Zeile 8 wird eine Konstante (in unserem Fall 10) in das Arbeitsregister 4 geladen. Anschließend bekommt das Register 6 ebenfalls einen Wert, dieser muss allerdings größer oder gleich \verb|10h| sein (dies hängt damit zusammen, alle Adressen vor diesem Wert vom internen Rom bereits belegt werden. Somit können dort keine Daten gespeichert werden.
...
In der Zeile 12 wird dann der Wert, der an der Adresse von R6 liegt, ausgelesen und in R5 geschrieben. Nun vergleicht man im Einzelschrittmodus manuell, ob dieser mit dem in \verb|R4| gespeicherten Wert "ubereinstimmt. Falls dies nicht der Fall sein sollte, so würde liegt ein Fehler im Speicher vorliegen.
...
\begin{psmatrix}[rowsep=0.4,colsep=0.5]
	...
	\ncline{->}{1,1}{2,1}
	\ncline{->}{2,1}{4,1}
	\ncangles[angleA=0, armA=0.5cm, armB=2.5cm]{->}{4,1}{Begin}\nbput{Ja}
	\ncline{->}{4,1}{5,1}\nbput{Nein}
	\ncline{->}{5,1}{6,1}
	\ncline{->}{6,1}{8,1}
	\ncline{->}{8,1}{10,1}
	\ncline{->}{10,1}{11,1}
	\ncangles[angleA=0, armA=0.5cm, armB=2.5cm]{->}{11,1}{Loop1}\nbput{Ja}
	\ncline{->}{11,1}{12,1}\nbput{Nein}
	\ncline{->}{12,1}{13,1}
	\ncline{->}{13,1}{14,1}\nbput{Nein}
	\ncline{->}{14,1}{15,1}
	\ncline{->}{15,1}{16,1}
	\ncangles[angleA=0, armA=1.5cm, armB=2.5cm]{->}{13,1}{Loop0}\nbput{Ja}
	\ncangles[angleA=-90, armA=0.5cm, armB=3.0cm]{->}{16,1}{5,1}
\end{psmatrix}
\end{pspicture}
\vspace{0.5cm}
\end{center}
