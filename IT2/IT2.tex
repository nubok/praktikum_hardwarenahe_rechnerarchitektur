...
Man kann in der Tat über all diese Adressen auf den externen RAM zugreifen. Jedoch tritt in der Praxis eine Besonderheit auf: wie man in der ersten Grafik von Anlage 1 der Versuchsbeschreibung sehen kann, sind die Adressbits A8 bis A12 mit Masse verbunden. Das heißt im RAM sind nur 256 (\verb|100h|) verschiedene Speicherzellen adressierbar. Die obersten 5 Adressbits (A8-A12) werden ignoriert. 
...
\psbrace[ref=lC](Init3)(Init2){\parbox{5cm}{R6 mit einer zulässigen Adresse initialisieren}}
...
In der Zeile 12 wird dann der Wert, der an der Adresse von \verb|R6| liegt, ausgelesen und in \verb|R5| geschrieben. Nun vergleicht man im Einzelschrittmodus manuell, ob dieser mit dem in \verb|R4| gespeicherten Wert "ubereinstimmt. Falls dies nicht der Fall sein sollte, so würde ein Fehler im Speicher vorliegen.
...
Gem"a"s den Instruktionen w"ahrend des Praktikums wurde die Aufgabenstellung derart erweitert, dass vor dem \verb|RETURN|-Befehl ein weiterer Sprung auf die Adresse \verb|600h| mit direkt folgendem Rücksprung erfolgen soll.
...
Anschließend muss Port 2 als Eingabe-Port initialisiert werden. Daher m"ussen im Steuerregister in \verb|P2M| alle Bits gesetzt werden, also \verb|FFh|, da alle Bits des Ports als Eingabe-Bits deklariert werden sollen.
...
Die L"osung ist damit naheliegend: nachdem eine Schalter"anderung detektiert wurde, soll der Microcontroller kurz warten, bis sich die Schwingungen beruhigt haben, und erst dann mit dem Programmablauf fortfahren. Dies wurde in dem Programm aus Listing \ref{lst:aufgabe32c} getan. Die zugehörige Registerbelegungstabelle ist Tabelle \ref{table:aufgabe32c} und das Flussdiagramm ist Abbildung \ref{abb:aufgabe32c}.
...
Anschließend muss das Timer-Mode-Register \verb|TMR| mit \verb|10011100| initialisiert werden. Hier erfolgt die richtige Belegung über die Eingangs- und Ausgangssignale (lt. Aufgabenstellung).

Danach werden die Initialwerte des Prescaler 1 (Wert 1) und des Timers 1 gesetzt, um dadurch die gewünschte Frequenz von 5 KHz zu bekommen. 

Der Prescaler wird außerdem über das auf 1 gesetzte Bit \verb|D0| in den Modulo N Modus und über das nicht gesetzte Bit \verb|D1| in den Input $T_{in}$ Modus gesetzt.

Letztendlich befindet sich im Programm eine Schleife, sodass das Programm nicht unerwünscht terminiert.

Der Grund, warum sich tats"achlich eine Frequenz von 5~kHz ergibt, l"asst sich aus Abbildung 5 des Aufgabenblattes entnehmen: die am \verb|OSC|-Eingang anliegende Frequenz wird erst einmal durch 2 dividiert. Anschließend wird sie in der Clock Logic nochmals durch  4 dividiert.
...
In Tabelle \ref{table:1} sind die Werte der Teilerfaktoren $PRE1_{TF}$ bzw. $T1_{TF}$, die zu initialisierenden Werte für die zugeh"origen Timerregister \verb|PRE1| bzw. \verb|T1|, die theoretisch zu erwartenden Werte f"ur $f_{T_{OUT}}$, sowie deren Messwerte abgetragen.
