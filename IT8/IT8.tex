...
\end{pspicture}
\end{center}
\caption{Bin"ar/BCD-Konvertierung für 7-Segment-Anzeige -- Detailbetrachtung Initialisierung}
...
\end{pspicture}
\end{center}
\caption{Bin"ar/BCD-Konvertierung für 7-Segment-Anzeige -- Detailbetrachtung Hauptschleife}
...
\begin{figure}
\lstinputlisting[caption={Assembler-Programm f"ur Bin"ar/BCD-Konvertierung für 7-Segment-Anzeige -- Funktion \texttt{moddiv10}}, language=z8asm, label=lsg:aufgabe33_3, firstline=64, firstnumber=64, lastline=78]{aufgabe33.asm}
\end{figure}
...
Die Hauptstruktur des Programms ist in Abbildung \ref{abb:aufgabe34_1} dargestellt. Der zugeh"orige Assembler-Code der Hauptschleife befindet sich in Listing \ref{aufgabe34_1}. Da die Initialisierung der Ports und des Stacks gegen"uber der vorherigen Aufgabe nicht ver"andert wurden, wird auf ein erneutes Abdrucken dieses Programmteile verzichtet.
...
\end{pspicture}
\end{center}
\caption{Bin"ar/BCD-Konvertierung für 7-Segment-Anzeige -- Hauptstruktur Programm}
\label{abb:aufgabe34_1}
\end{figure}

\begin{figure}
\lstinputlisting[caption={Assembler-Programm f"ur Zeitmultiplexbetrieb der ADU-Kanäle -- Hauptschleife}, language=z8asm, label=lsg:aufgabe34_1, firstline=13, lastline=22, firstnumber=13]{aufgabe34.asm}
\end{figure}

Wie man sieht, wurde das Auslesen des ADUs, die Verarbeitung und Ausgabe des Wertes in eine eigene Funktion \texttt{read\_loop} verlagert. Diese liest das Ergebnis des ADUs aus und macht geht anschließend in eine viersek"undige Warteschleife "uber.
...
Das Flussdiagramm des Aufbaus der Funktion \texttt{read\_loop} befindet sich in Abbildung \label{abb:aufgabe34_2}.
...
\end{pspicture}
\end{center}
\caption{Bin"ar/BCD-Konvertierung für 7-Segment-Anzeige -- Funktion \texttt{read\_loop}}
\label{abb:aufgabe34_2}
\end{figure}


Vom Betreuer wurde als zus"atzliche kleine Herausforderung verlangt, dass bei der Ausgabe 



\begin{figure}
\lstinputlisting[caption={Assembler-Programm f"ur Zeitmultiplexbetrieb der ADU-Kanäle -- Routine \texttt{read\_loop}}, language=z8asm, label=lsg:aufgabe34_1, firstline=25, lastline=42, firstnumber=25]{aufgabe34.asm}
\end{figure}

\begin{figure}
\lstinputlisting[caption={Assembler-Programm f"ur Zeitmultiplexbetrieb der ADU-Kanäle -- Routine \texttt{channel\_read\_and\_output}}, language=z8asm, label=lsg:aufgabe34_2, firstline=45, lastline=99, firstnumber=45]{aufgabe34.asm}
\end{figure}
